...
HookLengths[\[Lambda]_] := Select[Flatten[Differences /@ 
       Tuples[(Flatten[Position[PartitionToAbaci[\[Lambda]], #1]] & ) /@ 
         {1, 0}]], #1 > 0 & ]
 
HookFerrers[\[Lambda]_] := Show[Ferrers[\[Lambda]], HookCells[\[Lambda]]]
 
Ferrers[\[Lambda]_] := Graphics[RowOfCells @@@ 
      Transpose[{\[Lambda], Range[Length[\[Lambda]]]}], 
     ImageSize -> {UpTo[1000], 15*Length[\[Lambda]]}]
 
RowOfCells[L_, y_] := (OneCell[#1, y] & ) /@ Range[L]
 
OneCell[x_, y_] := Line[{{x, y}, {x + 1, y}, {x + 1, y + 1}, {x, y + 1}, 
      {x, y}}]
 
HookCells[\[Lambda]_] := Graphics[RowOfText @@@ 
      Transpose[{Reverse /@ HookRows[\[Lambda]], Range[Length[\[Lambda]]]}]]
 
RowOfText[L_, y_] := (Text[L[[#1]], {#1 + 0.5, y + 0.5}] & ) /@ 
     Range[Length[L]]
 
HookRows[\[Lambda]_] := Module[{a = Prepend[Accumulate[\[Lambda]], 0]}, 
     Table[HookLengths[\[Lambda]][[a[[i]] + 1 ;; a[[i + 1]]]], 
      {i, Length[a] - 1}]]
